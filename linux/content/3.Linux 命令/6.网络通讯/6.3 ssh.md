...
>  
    Host ali                            #别名
    HostName 192.168.1.2                #是目标主机的主机名或者IP地址。
    Port                                #指定的端口号
    User root                           #指定的登陆用户名
    PreferredAuthentications publickey  #强制使用Public Key验证
    IdentityFile ~/.ssh/ali             #指定的私钥地址
...
# PermitRootLogin no                          #禁止ROOT远程SSH登录， 启用后无法远程登陆
...
ServerAliveInterval 30              # 每30秒发送一次心跳检测
ServerAliveCountMax 10              # 超过10次都没有成功，就主动断开与服务器的连接
