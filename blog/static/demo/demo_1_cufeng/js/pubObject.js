...
function MovieClip2D( img, data ){
	...
	this.paint = function(){
		if( this.visible ){
			...
			context.rotate( this.rotate*Math.PI/180 ); //旋转角度
			context.scale( this.scaleX, this.scaleY ); //图像的比例
			switch(this.isPlay)
            {
                case 1:
                    context.drawImage(img,this.mcX,this.mcY,this.frameWidth,this.frameHeight,0,0,this.frameWidth,this.frameHeight);
                    break;
                case 2:
...
function Stage2D(){
    ...
    this.addObject = function(child){
		...
		}else{displayObjectList.splice( index, 1, child );
		}
	};
	this.removeObject = function(child){
		var idex = indexOf( child );
		if( index != -1 ){
